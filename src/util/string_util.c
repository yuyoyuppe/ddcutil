...
bool is_abbrev(const char * value, const char * longname, size_t minchars)
{
  ...
  if(value && longname)
  {
    ...
    if(vlen >= minchars && vlen <= strlen(longname) && memcmp(value, longname, vlen) == 0 // n. returns 0 if vlen == 0
    )
    {
      result = true;
    }
 ...
 *  @retval -1    no match
 */
int exactly_matches_any(const char * s, const char ** match_list) { return matches_by_func(s, match_list, streq); }
 ...
 * @return extracted substring, in newly allocated memory
 */
char * lsub(const char * s, int ct) { return substr(s, 0, ct); }
 ...
 */
char * chars_to_string(const char * start, size_t len)
{
  assert(len >= 0);
  char * strbuf = NULL;
  if(start)
...
bool sbuf_append(char * buf, int bufsz, char * sepstr, char * nextval)
{
  ...
  int  newlen    = (strlen(buf) == 0) ? strlen(nextval) : (strlen(buf) + seplen + strlen(nextval));
  if(newlen <= maxchars)
  {
 ...
 * to free the memory.
 */
char * hexstring2(const unsigned char * bytes, int len, const char * sepstr, bool uppercase, char * buffer, int bufsz)
{
  // if (len > 1)
 ...
 */
char * hexstring2_t_old(const unsigned char * bytes, int len, const char * sepstr, bool uppercase)
{
...
#ifdef DEPRECATED
char * hexstring2_t(const unsigned char * bytes, int len, const char * sepstr, bool uppercase)
{
                    ...
                    bool                  uppercase)                  // use upper case hex characters
{
  ...
  *buf = '\0';
  for(int i = 0; i < len; i++)
  {
    ...
    bool insert_sepstr =
      (hunk_size == 0) ? (i < (len - 1) && sepstr) : (i < (len - 1) && sepstr && (i + 1) % hunk_size == 0);
    if(insert_sepstr)
      strcat(buf, sepstr);
  }
  // printf("(%s) strlen(buffer) = %ld, required_size=%d   \n", __func__, strlen(buffer), required_size );
...
char * hexstring_t(const unsigned char * bytes, int len) { return hexstring3_t(bytes, len, " ", 1, false); }
 ...
 * @param   size     length of region
 */
void fhex_dump(FILE * fh, const Byte * data, int size) { fhex_dump_indented(fh, data, size, 0); }
 ...
 * @param   size     length of region
 */
void hex_dump(const Byte * data, int size) { fhex_dump(stdout, data, size); }
