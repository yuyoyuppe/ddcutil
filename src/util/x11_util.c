...
static void edid_recs_free_func(gpointer voidptr)
{
...
GPtrArray * get_x11_edids()
{
  ...
  if(disp)
  {
    ...
    if(XRRQueryExtension(disp, &event_base, &error_base) && XRRQueryVersion(disp, &maj, &min))
    {
                ...
                if(NULL != output_info)
                {
                  ...
                  if(RR_Connected == output_info->connection)
                  {
                    ...
                    for(size_t atom_count = 0; !edid_found && atom_count < atom_avail; ++atom_count)
                    {
                      ...
                      status = XRRGetOutputProperty(disp,
                                                    rr_output_id,
 ...
 */
void free_x11_edids(GPtrArray * edidrecs) { g_ptr_array_free(edidrecs, true); }
