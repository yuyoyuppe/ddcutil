 ...
 */
char * connector_type_name(Byte val) { return vnt_name(connector_type_table, val); }
 ...
 */
char * connector_type_title(Byte val) { return vnt_title(connector_type_table, val); }
...
Value_Name drm_property_flag_table[] = {VN(DRM_MODE_PROP_PENDING),   // (1<<0) == x01
...
{
#ifdef OLD
  interpret_named_flags_old(drm_property_flag_table, flags, buffer, bufsz,
                            ", "); //        sepstr);
#endif

  interpret_named_flags(flags,
...
Value_Name_Title drmModeConnection_table[] = {VNT(DRM_MODE_CONNECTED, "connected"),       //   1
 ...
 * @return symbolic name
 */
char * connector_status_name(drmModeConnection val) { return vnt_name(drmModeConnection_table, val); }
 ...
 * @return descriptive string
 */
char * connector_status_title(drmModeConnection val) { return vnt_title(drmModeConnection_table, val); }


Value_Name_Title drm_encoder_type_table[] = {VNT(DRM_MODE_ENCODER_NONE, "None"),       //  0
 ...
 * @return descriptive string
 */
char * encoder_type_title(uint32_t encoder_type) { return vnt_title(drm_encoder_type_table, encoder_type); }
...
static char * join_ids(char * buf, int bufsz, uint32_t * vals, int ct)
{
  buf[0] = '\0';
  if(ct > 0 && vals)
  {
...
void report_drmModeConnector(int fd, drmModeConnector * p, int depth)
{
  ...
  {
    rpt_vstring(d2,
                "index=%d, property id (props)=%u, property value (prop_values)=%u  0x%08x",
                ndx,
                p->props[ndx],
                p->prop_values[ndx],
                p->prop_values[ndx]);
 ...
 * @param  depth       logical indentation depth
 */
void report_property_value(int fd, drmModePropertyRes * prop_ptr, uint64_t prop_value, int depth)
{
  int d1 = depth + 1;
  ...
  else if(prop_ptr->flags & DRM_MODE_PROP_RANGE)
  {
    ...
    {
      rpt_vstring(
        d1, "Property value(range) = %d, min=%d, max=%d", prop_value, prop_ptr->values[0], prop_ptr->values[1]);
    }
  }
  ...
  {
    rpt_vstring(d1, "Object type, name = %s, value=%d", prop_ptr->name, prop_value);
  }
  ...
  else if(drm_property_type_is(prop_ptr, DRM_MODE_PROP_SIGNED_RANGE))
  {
    ...
    {
      rpt_vstring(d1,
                  "Property value(range) = %d, min=%d, max=%d",
                  (int64_t)prop_value,
                  (int64_t)prop_ptr->values[0],
                  (int64_t)prop_ptr->values[1]);
    }
  }
...
{
  rpt_vstring(
    depth, "Property %2d:  %-20s flags: 0x%08x - %s", p->prop_id, p->name, p->flags, interpret_property_flags(p->flags));
}
