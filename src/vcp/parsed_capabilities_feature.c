                                                       ...
                                                       size_t value_string_len)
{
  ...
  Capabilities_Feature_Record * vfr = (Capabilities_Feature_Record *)calloc(1, sizeof(Capabilities_Feature_Record));
  ...
  if(value_string_start)
  {
    ...
    {
      f0printf(
        FOUT, "Error processing VCP feature value list into bva_values: %.*s\n", value_string_len, value_string_start);
    }
#endif
...
void report_capabilities_feature(Capabilities_Feature_Record * vfr, DDCA_MCCS_Version_Spec vcp_version, int depth)
{
  ...
  rpt_vstring(
    d0, "Feature: %02X (%s)", vfr->feature_id, get_feature_name_by_id_and_vcp_version(vfr->feature_id, vcp_version));
  ...
  if(vfr->values)
  {
    ...
    DBGMSF(debug, "Feature values %sfound for feature 0x%02x", (feature_values) ? "" : "NOT ", vfr->feature_id);
    int ct = bva_length(vfr->values);
    if(feature_values)
    { // did we find descriptions for the features?
    ...
    { // no interpretation available, just show the values
