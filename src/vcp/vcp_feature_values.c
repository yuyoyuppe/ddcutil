...
void report_single_vcp_value(DDCA_Single_Vcp_Value * valrec, int depth)
{
  ...
  rpt_vstring(d1,
              "opcode=0x%02x, value_type=%s (0x%02x)",
              valrec->opcode,
              vcp_value_type_name(valrec->value_type),
              valrec->value_type);
    ...
    *buffer = '\0';
    if(valrec)
    {
      ...
      else
      {
        assert(valrec->value_type == DDCA_TABLE_VCP_VALUE);
        snprintf(
          buffer, bufsz, "opcode=0x%02x, value_type=Table, bytect=%d, ...", valrec->opcode, valrec->val.t.bytect);
        // easier just to convert the whole byte array then take what can fit
        char * buf0 = hexstring2(valrec->val.t.bytes,
                                 valrec->val.t.bytect,
                                 NULL, //   sepstr,
                                 true, //   uppercase,
...
void free_single_vcp_value_func(gpointer data) { free_single_vcp_value((DDCA_Single_Vcp_Value *)data); }

DDCA_Single_Vcp_Value * create_nontable_vcp_value(Byte feature_code, Byte mh, Byte ml, Byte sh, Byte sl)
{
...
DDCA_Single_Vcp_Value * create_cont_vcp_value(Byte feature_code, uint16_t max_val, uint16_t cur_val)
{
  DDCA_Single_Vcp_Value * valrec = calloc(1, sizeof(DDCA_Single_Vcp_Value));
  valrec->value_type             = DDCA_NON_TABLE_VCP_VALUE;
  valrec->opcode                 = feature_code;
...
DDCA_Single_Vcp_Value * create_table_vcp_value_by_bytes(Byte feature_code, Byte * bytes, uint16_t bytect)
{
  DDCA_Single_Vcp_Value * valrec = calloc(1, sizeof(DDCA_Single_Vcp_Value));
...
DDCA_Single_Vcp_Value * create_table_vcp_value_by_buffer(Byte feature_code, Buffer * buffer)
{
  return create_table_vcp_value_by_bytes(feature_code, buffer->bytes, buffer->len);
}


DDCA_Single_Vcp_Value * create_single_vcp_value_by_parsed_vcp_response(Byte feature_id, Parsed_Vcp_Response * presp)
{
  ...
  if(presp->response_type == DDCA_NON_TABLE_VCP_VALUE)
  {
    ...
    valrec = create_nontable_vcp_value(feature_id,
...
void free_vcp_value_set(Vcp_Value_Set vset) { g_ptr_array_free(vset, true); }

void vcp_value_set_add(Vcp_Value_Set vset, DDCA_Single_Vcp_Value * pval) { g_ptr_array_add(vset, pval); }

int vcp_value_set_size(Vcp_Value_Set vset) { return vset->len; }
