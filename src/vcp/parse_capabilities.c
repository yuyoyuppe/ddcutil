...
static void report_features(GPtrArray *            features, // GPtrArray of Capabilities_Feature_Record
...
void report_parsed_capabilities(Parsed_Capabilities * pcaps)
{
  ...
  DBGMSF(debug,
         "Starting. pcaps->raw_cmds_segment_seen=%s, pcaps->commands=%p, pcaps->vcp_features=%p",
         bool_repr(pcaps->raw_cmds_segment_seen),
         pcaps->commands,
         pcaps->vcp_features);
  ...
  bool damaged = false;
  rpt_vstring(d0, "MCCS version: %s", (pcaps->mccs_version_string) ? pcaps->mccs_version_string : "not present");
 ...
 */
Parsed_Capabilities * new_parsed_capabilities(char *           raw_value,
                                              char *           mccs_ver,
                                              ...
                                              GPtrArray *      vcp_features)
{
  ...
  pcaps->raw_cmds_segment_seen = raw_cmds_segment_seen, pcaps->commands = commands;
...
void free_parsed_capabilities(Parsed_Capabilities * pcaps)
{
  ...
  if(pcaps->vcp_features)
  {
    ...
    {
      Capabilities_Feature_Record * vfr = g_ptr_array_index(pcaps->vcp_features, ndx);
      // report_feature(vfr);
 ...
 */
static Capabilities_Segment * next_capabilities_segment(const char * start, int len)
{
  ...
  while(segment->name_start < pos && *segment->name_start == ' ')
    segment->name_start++;
...
static Byte_Value_Array parse_cmds_segment(const char * start, size_t len)
{
  bool debug = false;
...
static char * find_closing_paren(char * start, char * end)
{
  assert(*start == '(');
  char * pos   = start + 1;
  int    depth = 1;
 ...
 */
static GPtrArray * parse_vcp_segment(const char * start, size_t len)
{
  bool        debug     = false;
  ...
  bool         valid_feature;
  size_t       value_len   = 0;    // initialization logically unnecessary, but o.w. get warning
  const char * value_start = NULL; // ditto
  while(pos < end)
  {
    valid_feature = false;
 ...
 */
Parsed_Capabilities * parse_capabilities(const char * buf_start, size_t buf_len)
{
  ...
  // n. may be damaged
  Parsed_Capabilities * pcaps = new_parsed_capabilities(raw_value,
 ...
 *  @return pointer to newly allocated #Parsed_Capabilities structure
 */
Parsed_Capabilities * parse_capabilities_string(const char * caps) { return parse_capabilities(caps, strlen(caps)); }
...
Byte_Bit_Flags parsed_capabilities_feature_ids(Parsed_Capabilities * pcaps, bool readable_only)
{
  assert(pcaps);
  bool debug = false;
  DBGMSF(debug, "Starting. readable_only=%s, feature count=%d", bool_repr(readable_only), pcaps->vcp_features->len);

  Byte_Bit_Flags flags = bbf_create();
  if(pcaps->vcp_features)
  { // pathological case of 0 length capabilities string
    for(size_t ndx = 0; ndx < pcaps->vcp_features->len; ndx++)
    {
